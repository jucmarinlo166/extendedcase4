...
engine = create_engine('postgresql://postgres:CVlriAurUPkjAW8HzYEq@extendedcase4.cxikjfrtpv9g.us-east-2.rds.amazonaws.com/strategy')
df = pd.read_sql("SELECT * from trades", engine.connect(), parse_dates=('Entry time',))
df['YYYY-MM'] = pd.to_datetime(df['Entry time'].map(lambda x: x.strftime('%Y-%m')))
...
def update_start_date_range(value):
    new_df = df[df['Exchange'] == value]
    return new_df['Entry time'].min(), new_df['Entry time'].max()


def filter_df(exchange, margin, start_date, end_date):
    sql1 = df['Exchange'] == exchange
    sql2 = df['Margin'] == int(margin)
    sql3 = (df['Entry time'] >= start_date) & (df['Entry time'] <= end_date)
    sqlFinal = sql1 & sql2 & sql3
    return df.loc[sqlFinal]


def calc_returns_over_month(dff):
    out = []

    for name, group in dff.groupby('YYYY-MM'):
...
def update_pnlbar(exchange, leverage, start_date, end_date):
   dff = filter_df(exchange, leverage, start_date, end_date)
   bar1 = dff[dff['Trade type'] == 'Short']
   bar2 = dff[dff['Trade type'] == 'Long']
   data = [
       go.Bar(
           x=bar1['Entry time'],
           y=bar1['Pnl (incl fees)'],
           name='Short'
       ),
       go.Bar(
           x=bar2['Entry time'],
           y=bar2['Pnl (incl fees)'],
           name='Long'
       )
   ]
   layout = {
...
def update_linesChart1(exchange, leverage, start_date, end_date):
   dff = filter_df(exchange, leverage, start_date, end_date)
   data = [
       go.Scatter(
           x=dff['Entry time'],
           y=dff['BTC Price'],
           name='Daily BTC Price'
       ),
   ]
   layout = {
...
def update_linesChart2(exchange, leverage, start_date, end_date):
   dff = filter_df(exchange, leverage, start_date, end_date)
   data = [
       go.Scatter(
           x=dff['Entry time'],
